...
def make_media_database(dir_settings: dict) -> list:
    """メディアのデータベースを作成する"""
    log.debug('> def ' + sys._getframe().f_code.co_name)
    ...
    for item in dir_settings['ImageDir'].iterdir():
        if item.is_file():
            ...
            if item_size == True:
                ...
                # 実際に確認する
                expected_tmb_path = dir_settings['ThumbnailDir'] / ('tmb_' + item.stem + expected_suffix)
    ...
    #サムネイルディレクトリが無い場合は作る
    dir_settings['ThumbnailDir'].mkdir(parents=True, exist_ok=True)
    
    #現状サムネイルがいくつあるのか確認
    tmb_counter = 0
    for item in dir_settings['ThumbnailDir'].iterdir():
        if item.is_file():
            tmb_counter += 1
